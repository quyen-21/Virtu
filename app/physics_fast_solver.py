...
def fast_solve_layout(room, raw_items):
    ...
    rejected = []
    for it in items:
        cat = str(it.get('category', 'unknown')).lower()
    ...
    return items, rejected, {'fastScore': sc, 'penalties': pen, 'itemCount': len(items), 'rejectedCount': len(rejected), 'collisionCount': pen['collision'], 'outsideCount': pen['outside']}
