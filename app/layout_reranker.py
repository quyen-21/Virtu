def score_candidate_layout(items, rejected, metrics):
    ...
    s -= .04 * len(rejected) + .06 * p.get('collision', 0) + .08 * p.get('unsupported', 0) + .03 * p.get('wall', 0) + .05 * p.get('outside', 0)
    ...
    s -= .08 * float(metrics.get('collisionCount', 0)) + .08 * float(metrics.get('outsideCount', 0))
    if items:
...
def choose_best_layout(candidates):
    ...
    for c in candidates:
        c['metrics']['layoutScore'] = score_candidate_layout(c.get('items', []), c.get('rejected', []), c.get('metrics', {}))
